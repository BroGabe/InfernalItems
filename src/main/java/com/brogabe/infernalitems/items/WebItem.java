...
public class WebItem {
    ...
    public static ItemStack getItem(InfernalItems plugin, int amount) {
        ...
        List<String> lore = config.getStringList("web-item.lore");
        String name = config.getString("web-item.name");

        ItemStack itemStack = new ItemCreator(Material.EGG, name, 1, 0, "", lore).getItem();
        itemStack.setAmount(amount);

        NBTItem nbtItem = new NBTItem(itemStack);
        NBTCompound compound = nbtItem.getOrCreateCompound("InfernalItems");

        compound.setString("type", InfernalType.WEB.toString());
