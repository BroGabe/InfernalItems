...
public class BlinderBall {
    ...
    public static ItemStack getItem(InfernalItems plugin, int amount) {
        FileConfiguration config = plugin.getBlinderCFG().getConfig();

        List<String> lore = config.getStringList("blinder-item.lore");
        String name = config.getString("blinder-item.name");
        ...
        compound.setString("type", InfernalType.BLINDER.toString());
