...
public class BardItem {
    ...
    public static ItemStack getItem(InfernalItems plugin, int amount) {
        FileConfiguration config = plugin.getBardCFG().getConfig();

        List<String> lore = config.getStringList("bard-item.lore");
        String name = config.getString("bard-item.name");

        ItemStack itemStack = new ItemCreator(Material.GOLD_INGOT, name, 1, 0, "", lore).getItem();
        itemStack.setAmount(amount);

        NBTItem nbtItem = new NBTItem(itemStack);
        NBTCompound compound = nbtItem.getOrCreateCompound("InfernalItems");

        compound.setString("type", InfernalType.ANTI_BARD.toString());
